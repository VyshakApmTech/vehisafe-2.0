...
* Version      : CodeGenerator for RL78/G12 V2.04.04.02 [20 Nov 2019]
* Device(s)    : R5F10268
* Tool-Chain   : CCRL
...
{
    gFdlDesc.fx_MHz_u08 = 32;//_24_HOCO_CLOCK_MHz;   /* Set an integer of the range from 1 to 32 according to GUI setting of HOCO. */
...
{
	uint8_t i;
New_Write_Location=0x0000;//0xf1000

	DI();
	DFLEN=1;

	R_FDL_Open();
	R_FDL_Write(New_Write_Location, &IR_Cmd[0], 1023 );
	for(i=0;i<256;i++);
	
	EI();
	
}
Memory_Start_Addr=0xf1000;
void Read_Last_State(void)
{
	uint16_t i;
	DI();
	DFLEN=1;
	data_ptr=(uint8_t *)Memory_Start_Addr;
	if(*data_ptr!=0xff){
		//while(*data_ptr != 0xff)
		for(i=0;i<2048;i++)
		{
